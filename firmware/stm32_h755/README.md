...
- docelowo shared memory / message queue
...
- CM7 dodatkowo loguje na VCP przez `USART3` (`PD8/PD9`) bezpośrednim transmitem
- obecny bring-up nie używa jeszcze shared memory
- Nie uruchamiaj uploadu obu rdzeni równolegle na tym samym ST-LINK.
- Jeśli nie widzisz migania, najpierw sprawdź `LD1` i `LD2`.
- Jeśli nie widzisz logów, użyj monitora dla `CM7` i sprawdź port `/dev/tty.usbmodem24203`.
...
Firmware STM32H755 ma teraz stabilny baseline bring-up:

- CM7 miga `LD1` na `PB0`
- CM4 miga `LD2` na `PE1`
- komunikacja shared memory zostanie dodana później, etapami
